...
FandomLens는 YouTube Data API를 통해 K-POP 아티스트 관련 영상 데이터를 **자동 수집 → PostgreSQL 적재 → Claude AI 분석**까지 이어지는 엔드투엔드 데이터 파이프라인입니다.

SM엔터테인먼트가 추진하는 **자체 AI 서비스 백엔드 구축** 및 **글로벌 팬덤 데이터 활용** 전략에 직접 대응하는 구조로 설계했습니다.
